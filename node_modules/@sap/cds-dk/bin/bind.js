...
async function cds_bind(args, options) {
    const { on, 'to-app-services': app, to, out, 'output-file': outputFile, exec, credentials } = options
    ...
    if (on === 'k8s' && app) {
        throw `Option ${term.bold('--to-app-services')} is only supported for Cloud Foundry.`;
    }
    ...
    if (app) {
        ...
        const services = (await require('../lib/util/cf').services(app)).filter(s => Object.keys(s.credentials).length > 0)
