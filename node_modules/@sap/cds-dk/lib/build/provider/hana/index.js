...
const { compileToHdbMigration, getJournalEntities, getMigrationTableName } = require('./2migration')
const InternalBuildPlugin = require('../internalBuildPlugin')
const { BuildError, relativePaths, BuildMessage, hasOptionValue, getFiles, getWorkspacePaths } = require('../../util')
...
class HanaBuildPlugin extends InternalBuildPlugin {
    ...
    async build() {
        ...
        let wsPaths = []
        if (getJournalEntities(model).length || cds.cli.options?.[OPTION_WS] || hasOptionValue(this.task.options[OPTION_WS], true)) {
            // workspace paths don't include the current project root
            wsPaths = await getWorkspacePaths()
        }

        if (cds.cli.options.mocked) {
            cds.deploy?.include_external_entities_in?.(model)
        }

        // the order of 1 and 2 is important
        // 1. compile
        const hdiPlugins = await this._compileToHana(model, wsPaths)
        ...
        if (!this.hasBuildOption(OUTPUT_MODE, OUTPUT_MODE_NONE)) {
            ...
            // copy native hana artifacts from other workspaces including .hdbmigrationtable files defined for 
            // common model artifacts that are not part of the current project
            if (wsPaths.length || cds.cli.options?.[OPTION_WS] || hasOptionValue(this.task.options[OPTION_WS], true)) {
                // first is winning, e.g. a .hdiconfig file might exist in multiple workspaces
                const wsFiles = (await this._getWorkspaceFiles(wsPaths)).reduce((acc, src) => {
                    const { filePath } = src;
                    if (!acc.some(file => file.filePath === filePath)) {
                        acc.push(src);
                    }
                    return acc;
                }, []);

                // copy artifacts to the src/gen folder in inplace mode
                const dest = path.join(this.task.dest, this.isStagingBuild() ? 'src' : 'src/gen')
                await Promise.all(
                    wsFiles.map((src) => {
                        const { dir, filePath } = src
                        return this.copy(path.join(dir, filePath)).to(path.join(dest, filePath))
                    })
                )
            }
        }
    ...
    async _compileToHdbtabledata(model) {
        ...
        if (allCsvFiles.length > 0) {
            ...
            if (!toHdbtabledata) throw `hdbtabledata plugin not found – make sure to install package '@cap-js/hana'`
     ...
     */
    async _compileToHana(model, wsPaths) {
    ...
    async _compileToHdbmigration(model, format, wsPaths) {
        const hdiPlugins = new Set()
        ...
        const { definitions, afterImage } = await compileToHdbMigration(model, lastDev, dbSrcDir, options)
        let validationError

        // determine the migration tables originating from a different workspaces
        const journalEntities = getJournalEntities(model)
        const wsMigrationTables = new Set()
        if (journalEntities.length) {
            for (const entity of journalEntities) {
                if (entity['$location']?.file && wsPaths.some(wsPath => path.resolve(cds.root, entity['$location'].file).startsWith(wsPath))) {
                    wsMigrationTables.add(getMigrationTableName(entity, model))
                }
            }
        }
        ...
        for (const { name, suffix, content, changed } of definitions) {
            ...
            if (suffix === FILE_EXT_HDBMIGRATIONTABLE) {
                // do not create .hdbmigrationtable files originating from a different workspace
                // the --ws build option will copy these artifacts to either gen/db/src or db/src/gen
                if (!wsMigrationTables.has(name)) {
                    migrationTableFiles.push(path.join(relDbSrcDir, fileName))
                    if (changed) {
                        if (this.hasBuildOption(OUTPUT_MODE, OUTPUT_MODE_RESULT)) {
                            this._result.hana.add(path.join("src", fileName))
                        }
                        promises.push(this.write(content).to(path.join(dbSrcDir, fileName)))
                    } else {
                        DEBUG?.(`no change, keep existing ${fileName}`)
                    }
                } else {
                    DEBUG?.(`skip generation of migration table [${fileName}] as it originates from a different workspace`)
                }
            } else {
                if (this.hasBuildOption(OUTPUT_MODE, OUTPUT_MODE_RESULT)) {
                    this._result.hana.add(path.join(relDestDir, fileName))
                }
                promises.push(this.write(content).to(path.join(this.task.options.compileDest, fileName)))
                if (suffix === FILE_EXT_HDBTABLE) {
                    // issue an error in case a .hdbmigrationtable file already exists
                    if (fs.existsSync(path.join(dbSrcDir, name + FILE_EXT_HDBMIGRATIONTABLE))) {
                        validationError = new BuildError(`Multiple files exist defining the same HANA artifact - [${path.join(relDbSrcDir, name + FILE_EXT_HDBMIGRATIONTABLE)}, ${path.join(relDbDestDir, fileName)}].\nEither annotate the model entity using @cds.persistence.journal or undeploy the file [${path.join('src', name + FILE_EXT_HDBMIGRATIONTABLE)}] using an undeploy.json file.`)
                    }
                }
            }
        }
    ...
    async _writeHdiConfig(compileHdiPlugins) {
        const DEFAULT_HDI_PLUGINS = [FILE_EXT_CSV, FILE_EXT_HDBTABLEDATA, FILE_EXT_HDBTABLE, ".hdbview", ".hdbindex", ".hdbconstraint", ".hdbcalculationview"]
        ...
        if (Object.keys(content['file_suffixes']).length < hdiPlugins.size) {
            const missingPlugins = Array.from(hdiPlugins).filter(key => !content['file_suffixes'][key.slice(1)])
            this.pushMessage(`'HANA database plugin not found for following file suffix [${missingPlugins.join(', ')}]`)
        }
    ...
    /**
     * Returns all hana native artifacts located in db/src/** workspace folders.
     * @param {Array} workspacePaths Array of workspace paths, 'cds.root' is excluded from the list
     * @returns {Array} Array of workspace files, each entry contains the db src directory, 
     * the relative file path and the file name.
     */
    async _getWorkspaceFiles(workspacePaths) {
        if (!workspacePaths || !workspacePaths.length) {
            return []
        }
        const dbPaths = new Set()
        workspacePaths.forEach(workspacePath => {
            // skip workspace matching the current project if cds build is executed in a workspace folder
            if (cds.root !== workspacePath) {
                const dbSrc = path.join(workspacePath, 'db/src')
                const dbCfg = path.join(workspacePath, 'db/cfg')
        ...
        const workspaceFiles = [...dbPaths].reduce((acc, src) => {
            const files = getFiles(src, (entry) => {
                if (entry.match(/(\/|\\)gen(\/|\\)?$/)) {
            ...
            files.forEach(file => acc.push({ dir: src, filePath: path.relative(src, file), fileName: path.basename(file) }))
            return acc
        }, [])
    ...
    _hasJournalDefinitions(model) {
        return Object.values(model.definitions).some(def => def['@cds.persistence.journal'])
    }
