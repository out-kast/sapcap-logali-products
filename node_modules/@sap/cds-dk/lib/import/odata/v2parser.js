...
function _getServiceComplexTypes(parserContext) {
	...
	for (let i = 0; i < complexTypesKeys.length; i++) {
		...
		//S4 might have actions or functions which return a complex type named like the action or function. In this case the complex type must be renamed as else a compile error happens as the result type cannot be found / is a function.
		if (parserContext.allFunctionImports.some(fi => fi[0] === complexTypeKey)) {
			const originalName = complexTypeKey;
			const functionImport = parserContext.allFunctionImports.find(fi => fi[0] === complexTypeKey);
			complexTypeKey = functionImport[0] + 'ReturnType';
			let i = 1;
			while (
				parserContext.allFunctionImports.some(fi => fi[0] === complexTypeKey) ||
				parserContext.allEntitySetMapMC[complexTypeKey] ||
				parserContext.allEntitySetMap[_extractEntityFromNamespace(complexTypeKey)] ||
				parserContext.allEntities.includes(_extractEntityFromNamespace(complexTypeKey))
			) {
				if (i > 1) {
					complexTypeKey = complexTypeKey.slice(0, complexTypeKey.length-1)
				}
				complexTypeKey += i;
				i++;
			}
			
			//Rename complexType in parserContext for downstream FunctionImport logic
			parserContext.allComplexTypes[complexTypeKey] = parserContext.allComplexTypes[originalName];
			parserContext.allComplexTypes[complexTypeKey]['_attributes'].Name = complexTypeKey
			delete parserContext.allComplexTypes[originalName]
			//Correct Return Type of affected FunctionImports
			const functionImportsWithWrongReturnType = parserContext.allFunctionImports.filter(fi => fi[2] === originalName || fi[2] === `Collection(${originalName})`);
			for (const functionImport of functionImportsWithWrongReturnType)  {
				functionImport[2] = functionImport[2].replace(originalName, complexTypeKey);
			}
		}

		complexTypeCSN += _getServiceComplexType(complexTypeKey, complexType, parserContext, isOpenType);
...
function _generateEDMXJSON2CSN(jsonObj, ignorePersistenceSkip, parserContext) {
	...
	// if entityJson exists(ignoring the case of no entity) and is not an array, convert it to an array to handle the case of single entity	
	if (entityJson && !Array.isArray(entityJson)) entityJson = [entityJson];
	const definitions = [];
	...
	// if there are unbound function imports
	if (Object.keys(parserContext.allFunctionImports).length > 0) {
		// passing 0 for unbound actions/functions and 0 for entityKeysList 
		definitions.push(
