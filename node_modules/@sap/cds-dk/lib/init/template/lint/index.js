...
module.exports = class LintTemplate extends require('../../plugin') {
  ...
  async run() {
    ...
    const ALLOWED_FILE_EXTENSIONS = ['*.cds', '*.csn', '*.csv']
    const extensions = new Set(ALLOWED_FILE_EXTENSIONS.map((ext) => path.extname(ext).replace('.', '')))
    for (const ext of extensions) {
      if (ext in VSCODE_FILE_ASSOCIATIONS) {
        for (const assoc of VSCODE_FILE_ASSOCIATIONS[ext]) {
